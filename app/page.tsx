...
import type { SavingsRecord, Goal } from '@/src/lib/types';
...
const SplashScreen = () => (
  <div className="fixed inset-0 z-50 grid place-items-center app-shell-bg">
    ...
    <style jsx>{`
      ...
      .animate-zoomIn { animation: zoomIn 0.9s ease; }
    `}</style>
  </div>
);

const PersonalizedTip = ({ history, trigger }: { history: SavingsRecord[], trigger: number }) => {
...
const SavingsHistoryList = ({ history, onSavePostponed }: { history: SavingsRecord[], onSavePostponed: (day: number) => void }) => {
    ...
    <div className="space-y-3">
      {history.map((item) => (
        <div key={item.day} className="glass card-radius p-3 flex items-center gap-3">
          <div className={`h-11 w-11 rounded-2xl grid place-items-center border
            ${item.saved ? 'bg-[rgba(162,227,147,0.12)] border-[rgba(162,227,147,0.18)] text-[rgb(162,227,147)]'
          ...
          <div className="flex items-center gap-2">
            ...
            {!item.saved ? (
              <Button size="sm" variant="outline" className="pill border-white/15 bg-white/5 hover:bg-white/10" onClick={() => onSavePostponed(item.day)}>
                Ahorrar
              </Button>
            ) : null}
          </div>
            ...
            <Dialog>
              ...
              <DialogContent className="glass border-white/10">
                ...
                <div className="text-sm text-muted-foreground space-y-2">
                  ...
                  <p><strong>Ahorrar:</strong> Guarda el monto mostrado. ¡Felicidades!</p>
              ...
              <AlertDialogContent className="glass border-white/10">
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    Esta acción no se puede deshacer. Todo tu progreso será eliminado.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                  <AlertDialogCancel className="pill">Cancelar</AlertDialogCancel>
                  <AlertDialogAction className="pill" onClick={() => { onReset(); setIsOpen(false); }}>
                    Reiniciar
                  </AlertDialogAction>
                </AlertDialogFooter>
              </AlertDialogContent>
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const usedCards = new Set(history.map((item: SavingsRecord) => item.value));
  ...
  const goalsProgress = useMemo(
    () => totalGoalsAmount > 0 ? (totalSavings / totalGoalsAmount) * 100 : 0,
    [totalSavings, totalGoalsAmount]
  );
  ...
  // ✅ Bloquea sacar tarjeta si ya hay una en pantalla
  const handleDrawCard = useCallback(() => {
    if (currentDrawnNumber !== null) return; // 🔒 ya hay tarjeta pendiente
    if (remainingCards.length === 0) return;
    const randomIndex = Math.floor(Math.random() * remainingCards.length);
    setCurrentDrawnNumber(remainingCards[randomIndex]);
  }, [remainingCards, currentDrawnNumber]);

  const handleCardAction = useCallback((saved: boolean) => {
    ...
    setTipTrigger(c => c + 1);
  }, [currentDrawnNumber, savingsHistory]);
  ...
  const handleSavePostponed = useCallback((day: number) => {
    setSavingsHistory(prev =>
      prev.map(item =>
        item.day === day ? { ...item, saved: true, date: new Date().toISOString().split('T')[0] } : item
      )
    );
  }, []);
    ...
    <header className="flex items-center justify-between gap-3">
      {screen !== 'main' ? (
        <Button
          variant="ghost"
          size="icon"
          className="pill bg-white/5 hover:bg-white/10 border border-white/10"
    ...
    <div className="mx-auto max-w-md px-4 pt-6 pb-28 space-y-5">
      ...
      <div className="glass card-radius p-4">
        <div className="flex justify-between mb-2 text-xs text-muted-foreground">
          <span>Progreso</span>
          <span className="font-semibold text-foreground">{drawnCount}/{TOTAL_DAYS}</span>
        </div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            ...
            >
              {currentDrawnNumber !== null
                ? 'Decide primero'
                : remainingCards.length > 0 ? 'Sacar tarjeta' : 'Completado'}
            </Button>

            <Button
              variant="outline"
              className="pill border-white/15 bg-white/5 hover:bg-white/10"
              onClick={() => setScreen('real_savings')}
            >
      ...
      {currentDrawnNumber !== null && (
        <div className="glass neon-outline card-radius p-5">
          ...
          <p className="mt-2 text-6xl font-black text-[hsl(var(--primary))] tabular-nums">
            ${currentDrawnNumber}
          </p>
          ...
          <div className="mt-5 grid grid-cols-2 gap-3">
            ...
            <Button
              variant="secondary"
              className="pill font-semibold bg-white/5 hover:bg-white/10 border border-white/10"
          ...
          <p className="mt-3 text-xs text-muted-foreground">
            Tip: si la pospones, podrás completarla en “Ahorro Real”.
          </p>
        </div>
      )}
            ...
            <div className="space-y-3">
              {[...savingsHistory].slice(-3).reverse().map((item) => (
                <div key={item.day} className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`h-9 w-9 rounded-2xl grid place-items-center border
                      ${item.saved ? 'bg-[rgba(162,227,147,0.12)] border-[rgba(162,227,147,0.18)] text-[rgb(162,227,147)]'
                        : 'bg-[rgba(255,80,80,0.10)] border-[rgba(255,80,80,0.16)] text-[rgb(255,120,120)]'}`}>
        ...
        <div className="mt-4">
          <Tabs defaultValue="monthly" className="w-full">
            <TabsList className="grid w-full grid-cols-2 bg-white/5 border border-white/10 pill">
              <TabsTrigger value="daily" className="pill">Diario</TabsTrigger>
              <TabsTrigger value="monthly" className="pill">Mensual</TabsTrigger>
            </TabsList>
  ...
  const ProfileScreen = (
    <div className="mx-auto max-w-md px-4 pt-6 pb-28 space-y-5">
      ...
      {/* ✅ Form de edición (se enfoca cuando vienes del botón Editar) */}
      <div className="glass neon-outline card-radius p-5">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-2">
              Si subes imagen, se guardará local (base64) y este campo quedará vacío.
            </p>
          </div>
        ...
        <Dialog>
          ...
          <DialogContent className="glass border-white/10">
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
                ...
                <div className="flex gap-2 mt-2">
                  {goalColors.map(color => (
                    <button
                      ...
                      className={`w-9 h-9 rounded-full transition-all border border-white/15
                        ${selectedColor === color ? 'ring-2 ring-[hsl(var(--primary))] ring-offset-2 ring-offset-black' : ''}`}
              ...
              <Button type="submit" className="w-full pill">Guardar</Button>
            </form>
          </DialogContent>
        </Dialog>
